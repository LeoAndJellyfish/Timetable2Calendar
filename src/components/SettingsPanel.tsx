...
export function SettingsPanel({ settings, setSettings, section, courses, onDone }: { settings: Settings; setSettings: (settings: Settings) => void; section: 'settings' | 'exceptions'; courses: Course[]; onDone: () => void }) {
  ...
  return <section className="panel settings-panel">
    ...
    <label className="field-label calendar-format-field">日历事件格式<select value={settings.eventDetails} onChange={event => update('eventDetails', event.currentTarget.value as Settings['eventDetails'])}><option value="concise">简洁：课程名 / 教室 / 教师</option><option value="detailed">详细：附周次、节次、教学班和学分</option></select><small>简洁格式把教师放在备注中、地点只保留教室。学分和课程备注在详细格式中显示；调课和待筛选提示始终保留。</small></label>
