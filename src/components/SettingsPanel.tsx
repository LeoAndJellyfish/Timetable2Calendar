import { CalendarDays, Clock3, Plus, Trash2, ArrowRight, Check } from 'lucide-react';
...
export function SettingsPanel({ settings, setSettings, section, courses, onDone }: { settings: Settings; setSettings: (settings: Settings) => void; section: 'settings' | 'exceptions'; courses: Course[]; onDone: () => void }) {
  ...
  if (section === 'exceptions') return <section className="panel settings-panel">
    <div className="section-heading"><h1>停课与调课</h1><CalendarDays size={24} aria-hidden="true" /></div>
    <label className="field-label">整日停课日期<textarea rows={3} placeholder={'例如：2026-10-01\n2026-10-02'} value={settings.excludedDates} onInput={event => update('excludedDates', event.currentTarget.value)} /><small>每行一个日期，或用逗号分隔；当天全部课程停课。</small></label>
    <div className="subheading"><h2>整日调课</h2><button className="button secondary compact" onClick={() => update('changes', [...settings.changes, { id: crypto.randomUUID(), from: '', to: '' }])}><Plus size={15} />添加规则</button></div>
    <p className="muted">将原日期的全部课程移至新日期，节次不变；新日期已有课程保留。</p>
    ...
    {settings.changes.length > 0 && <p className="muted footnote">多条规则同时生效，不连续转移；新日期若为停课日，移入课程也不导出。</p>}
  ...
  return <section className="panel settings-panel">
    <div className="section-heading"><h1>学期与时间</h1><Clock3 size={24} aria-hidden="true" /></div>
    ...
    <div className="form-grid">
      ...
      <label className="field-label">时区<div className="static-input">中国标准时间 · UTC+08:00</div></label>
    </div>
    <label className="field-label calendar-format-field">日历事件格式<select value={settings.eventDetails} onChange={event => update('eventDetails', event.currentTarget.value as Settings['eventDetails'])}><option value="concise">简洁：课程名 / 教室 / 教师</option><option value="detailed">详细：附周次、节次、教学班和学分</option></select><small>详细格式还包含课程备注；两种格式都保留调课和待筛选提示。</small></label>
    <div className="subheading"><h2>每天的作息时间</h2><button className="text-button" onClick={() => update('periods', initialSettings().periods)}>恢复中财默认</button></div>
    <p className="muted">中财默认作息，每节 45 分钟，可逐项修改。</p>
