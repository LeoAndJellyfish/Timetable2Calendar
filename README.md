...
1. **获取课表。** 通过课历「使用指南」顶部的「打开教务课表」链接进入教务系统，选择学年与学期。导出文字版 PDF，或在「表格」视图等课程显示完整后，按 Ctrl + S 保存为「网页，全部」或「网页，仅 HTML」。已有课历 JSON 备份也可直接导入。
...
PDF 和 HTML 中的学分会一并读取，支持 `0.25` 等小数，可在「核对课程」中修改。详细模式将学分写入日历备注，JSON 备份将其保存为数值字段 `credits`。未提供学分时留空，旧版无此字段的备份仍可导入。
...
界面采用新拟物风格，以暖白陶瓷面板和石墨按键为主，输入区使用内阴影，课程卡片保留浅色底与清晰文字。字体和资源均随站点部署。默认首页显示简短用途说明和导入入口，导入后展示课程统计、设置和预览。「学期与时间」「停课与调课」页面直接展示设置项，必要提示放在对应控件旁。支持手机布局和减少动态效果的系统设置。
...
常规测试检查 HTML / JSON 提取与 PDF 版面解析，涵盖学分读取、备份回读、分段与单双周、异常输入，以及停调课和冲突处理；也检查时区和 ICS 规范，使用独立的 `ical.js` 回读日历。构建回归测试检查 PDF worker 的输出扩展名、内容与引用。
